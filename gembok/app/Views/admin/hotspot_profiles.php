<?php
/**
 * MikroTik Hotspot Profiles View
 */
$profilesData = $profiles ?? [];
?>
<?= $this->extend('layout') ?>

<?= $this->section('title') ?>Profile Hotspot - Gembok Admin<?= $this->endSection() ?>
<?= $this->section('page_title') ?>Profile Hotspot<?= $this->endSection() ?>

<?= $this->section('content') ?>
<!-- Profiles Table -->
<div class="card">
    <div class="card-header">
        <h3 class="card-title"><i class="fas fa-wifi"></i> Daftar Profile Hotspot</h3>
        ...
        <tbody>
            ...
            <?php else: ?>
                ...
                <tr>
                    ...
                    <td><?= esc($profile['session-timeout'] ?? '-') ?></td>
                    <td><?= esc($profile['idle-timeout'] ?? '-') ?></td>
                    <td><?= esc($profile['shared-users'] ?? '1') ?></td>
                    <td>
...
<div id="profileModal" style="display: none; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.8); z-index: 2000; align-items: center; justify-content: center;">
    <div class="card" style="width: 550px; max-width: 90%; margin: 2rem;">
        <div class="card-header">
            <h3 class="card-title" id="modalTitle"><i class="fas fa-plus-circle"></i> Tambah Profile Hotspot</h3>
        ...
        <form id="profileForm" onsubmit="submitProfile(event)">
            ...
            <div class="form-group">
                ...
                <input type="text" id="profile_name" class="form-control" placeholder="Contoh: voucher-1jam" required>
            </div>
            <div class="form-group">
                <label class="form-label">Rate Limit (Upload/Download)</label>
                <input type="text" id="rate_limit" class="form-control" placeholder="Contoh: 2M/5M">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                <div class="form-group">
                    ...
                    <input type="text" id="session_timeout" class="form-control" placeholder="Contoh: 1h, 1d, 30m">
                    <small style="color: var(--text-muted);">Durasi sesi maksimal</small>
                </div>
                <div class="form-group">
                    <label class="form-label">Idle Timeout</label>
                    <input type="text" id="idle_timeout" class="form-control" placeholder="Contoh: 5m, 10m">
                    <small style="color: var(--text-muted);">Batas waktu idle</small>
                </div>
            </div>
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                <div class="form-group">
                    <label class="form-label">Shared Users</label>
                    <input type="number" id="shared_users" class="form-control" value="1" min="1" max="100">
                    <small style="color: var(--text-muted);">Login bersamaan</small>
                </div>
                <div class="form-group">
                    <label class="form-label">Keepalive Timeout</label>
                    <input type="text" id="keepalive_timeout" class="form-control" placeholder="Contoh: 2m">
                </div>
            </div>
...
function openAddModal() {
    ...
    document.getElementById('modalTitle').innerHTML = '<i class="fas fa-plus-circle"></i> Tambah Profile Hotspot';
...
function openEditModal(name) {
    ...
    document.getElementById('modalTitle').innerHTML = '<i class="fas fa-edit"></i> Edit Profile Hotspot';
    document.getElementById('profile_name').value = profile.name || '';
    document.getElementById('rate_limit').value = profile['rate-limit'] || '';
    document.getElementById('session_timeout').value = profile['session-timeout'] || '';
    document.getElementById('idle_timeout').value = profile['idle-timeout'] || '';
    document.getElementById('shared_users').value = profile['shared-users'] || '1';
    document.getElementById('keepalive_timeout').value = profile['keepalive-timeout'] || '';
    document.getElementById('resultMsg').style.display = 'none';
    document.getElementById('profileModal').style.display = 'flex';
}
...
async function submitProfile(e) {
    ...
    const data = {
        action: mode === 'edit' ? 'edit_hotspot_profile' : 'add_hotspot_profile',
        ...
        shared_users: document.getElementById('shared_users').value,
        keepalive_timeout: document.getElementById('keepalive_timeout').value
    };
...
function deleteProfile(name) {
    ...
    fetch('<?= base_url('admin/mikrotik/action') ?>', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'delete_hotspot_profile', name: name })
    })
    .then(res => res.json())
    .then(result => {
